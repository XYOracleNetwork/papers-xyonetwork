...
\title {XYO Network in Rental Car Facilities}
...
The \Gls{xyo-network} has already experimented with the implementation of their location network at airports in order to prevent customers from accidentally leaving without returning the keys to their rental car. By adding XYO Network \Glspl{bridge} to the system, rental car companies can have \gls{trustless}, independent confirmation that keys are returned.  An accessible ledger will not only help prevent loss, but help locate lost keys more efficiently in the case of misplacement. It's easy to imagine the larger scope of such a technology. The instillation of XYO Network \Glspl{sentinel} and Bridges in vehicles also serves as an excellent fleet management tool.
