...
The XYO Network is a trustless system that is designed to take unreliable Internet of Things devices and provide a trustworthy location. In this document we will detail out the implementation of this system from a technical perspective. Modeled after a patent application, the goal is if you followed the document you could re-create the XYO Network. 

\section{Network Overview}
\includegraphics [width=\textwidth]{XYONetwork}
When a Query is created it communicates with a network of Diviners. They instigate a request down the network querying devices for the given ID. When responses come back from the network it calculates the most trusted data point and returns it to the Querying party. 
