...
\begin{itemize}
  \item XNS & Names both resolve to addresses
  \item XNS Names can be used anywhere in the path as they're globally unique (with respect to a trusted nameserver)
  \item Names can can only be used nested under an address as they're not globally unique
  \item Given a path, the algorithm is to walk resolution of each of the immediate child paths with respect to the resolved parent
  \item You can omit path segments to maintain ambiguity of internal implementation
\end{itemize}
